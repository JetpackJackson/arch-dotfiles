...
local function test()
    ...
    vim.cmd(":wincmd l")
    vim.cmd(":terminal")
    --vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes(":terminal",true,false,true),"m", false)
    vim.api.nvim_feedkeys(py_cmd, "t", false)
end
...
local function debugger()
    ...
    -- Get terminal codes for running python file
    -- ("i" to enter insert before typing rest of the command)
    local py_cmd = vim.api.nvim_replace_termcodes("ipython -m pdb \"" .. file_name .. "\"<cr>", true, false, true)
